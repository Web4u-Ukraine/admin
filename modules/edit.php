<div class="col-md-12">
	<div class="widget stacked">
	    <div class="widget-header">
		  	...
		  	<h3><?= $page[$arg2][edit] ?></h3>
	    </div> <!-- /.widget-header -->
		<?
		$info=sql(1, "$arg2", "where id='$_REQUEST[id]'");
		$q=$info[0];
		?>
		<div class="widget-content">
			<form action="/" role="form" class="form-horizontal col-md-12" id="edit-index" data-table="<?= $arg2 ?>">
				<div id="wrap">
								...
								switch ($table[$arg2][$row[0]][type]){
									case 'input':
										...
										<div class="form-group">
											...
											<div class="col-md-6">
												<input type="text" class="form-control <? if ($row[0]=='data') echo 'datepicker'; ?>" name="<?= $row[0] ?>" value="<?= $q[$row[0]] ?>">
											</div>
										</div> <!-- /.form-group -->
										<?
										break;
									...
									case 'hidden':
										?>
										<input type="hidden" name="<?= $row[0] ?>" value="<?= $q[$row[0]] ?>">
										<?
										break;
											...
											<div class="col-md-6">
												<select name="<?= $row[0] ?>" class="form-control" <? if ($table[$arg2][$row[0]][change]!="") echo 'onchange="'.$table[$arg2][$row[0]][change].'"'; ?>>
													...
													foreach ($option as $label){
														?>
														<option value="<?= $label[$value] ?>" <? if ($label[$value]==$q[$row[0]]) echo 'selected'; ?>><?= $label[$text] ?></option>
														<?
													} }
													?>
												</select>
									...
									case 'textarea':
										...
										<div class="form-group">
											<label class="col-md-2"><?= $table[$arg2][$row[0]][name] ?></label>
											<div class="col-md-10">
												<textarea class="form-control <?= $table[$arg2][$row[0]]['class'] ?>" rows="5" name="<?= $row[0] ?>"><?= $q[$row[0]] ?></textarea>
											</div>
										</div>
										<?
										break;
										...
										<div class="form-group">
											...
											<div class="col-md-10">
												<ul class="gallery-container" data-name="<?= $row[0] ?>">
													<?
													$temp=explode('&', $q[$row[0]]);
													foreach ($temp as $t){
														?>
														<li><a href="/source/<?= $arg2 ?>/<?= $t ?>" class="ui-lightbox"><img src="/timthumb.php?src=/source/<?= $arg2 ?>/<?= $t ?>&zc=2&w=185&h=125" alt="" /></a><a href="/source/<?= $arg2 ?>/<?= $t ?>" class="preview"></a><input type="hidden" value="<?= $t ?>" name="<?= $row[0] ?>[]"><i class="fa fa-times fa-2x delete-load-photo"></i></li>
														<?
													}
													?>
												</ul>
								...
								switch ($table[$arg2][$row[0]][type]){
									...
									case 'textarea':
										...
										<div class="form-group">
											<label class="col-md-2"><?= $table[$arg2][$row[0]][name] ?></label>
											<div class="col-md-6">
												<textarea class="form-control" rows="5" name="<?= $row[0] ?>"><?= $q[$row[0]] ?></textarea>
											</div>
										</div>
										<?
										break;
